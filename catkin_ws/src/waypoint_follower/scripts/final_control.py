...
class WaypointFollower():
    def __init__(self):
        ...
        # remote signal for start
        self.rs           = 0
        self.sub_rs       = rospy.Subscriber('/remote_signal', Bool, self.callback_rs, queue_size = 1)
    
    def callback_rs(self, msg):
        self.rs = 1
        #rospy.loginfo('remote_good')
    ...
    def calc_error(self, ego_x, ego_y, ego_yaw, x_list, y_list, obstacle_x_list, obstacle_y_list, wpt_look_ahead=0):
        ...
        anglesum = 0
        index_ = 10
        if (near_ind + 1 == len(x_list)):
            anglesum = math.atan2(y_list[near_ind + index_ - len(x_list)] - y_list[near_ind], x_list[near_ind + index_ - len(x_list)] - x_list[near_ind])- math.atan2(y_list[0] - y_list[near_ind], x_list[0] - x_list[near_ind])
        elif (near_ind + index_ >= len(x_list)):
            anglesum = math.atan2(y_list[near_ind + index_ - len(x_list)] - y_list[near_ind], x_list[near_ind + index_ - len(x_list)] - x_list[near_ind])- math.atan2(y_list[near_ind + 1] - y_list[near_ind], x_list[near_ind + 1] - x_list[near_ind])
        else:
            anglesum = math.atan2(y_list[near_ind + index_] - y_list[near_ind], x_list[near_ind + index_] - x_list[near_ind]) - math.atan2(y_list[near_ind + 1] - y_list[near_ind], x_list[near_ind + 1] - x_list[near_ind])
        if (abs(anglesum) > 0.26): self.Kp = 0.72
        elif (abs(anglesum) > 0.13): self.Kp = 0.5
        else: self.Kp = 0.3
        #if (ego_x < -0.3 and ego_x > -5.37):
        #    self.Kp = 0.6
    ...
    def speed_control(self):
        if self.rs == 0:
            ...
            #rospy.loginfo('speed_control_hi')
            pass
